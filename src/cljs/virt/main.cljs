  ...
  (reify
    ...
    (will-mount [_]
      (let [nav-chan (listen-navigation)]
        (go (while true
          (om/update! data [:page-stack] (path-to-stack routes (<! nav-chan))))))
          ...
          (while true
            (let [[msg msg-data] (<! comm)]
              (case msg
                ...
                :navigate
                (let [page-stack (:page-stack @data)
                      [nav-type page-params] msg-data
                      params (merge page-params (second (peek page-stack)))
                      new-stack
                      (case nav-type
                        :back (pop page-stack)
                        (conj page-stack [nav-type params]))]
                  (om/update! data [:page-stack] new-stack)
        ...
        (dom/div nil
          ...
          (dom/div #js {:id "content"}
            (case page
              ...
              :home (om/build channel-list (:channels data) m)
              :virt.chat/home (om/build chat/main (:chat data) m))))))))
